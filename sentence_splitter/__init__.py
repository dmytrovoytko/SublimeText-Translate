...
class SentenceSplitter(object):
    ...
    def __init__(self, language: str, non_breaking_prefix_file: str = None):
        ...
        # VDI +++ Loading from Sublime Package
        try:
            import sublime
            if non_breaking_prefix_file is None:
                try:
                    non_breaking_prefix_file = sublime.find_resources('{}.txt'.format(language))
                    #print('non_breaking_prefix_file', non_breaking_prefix_file[0])
                    if not non_breaking_prefix_file:
                        raise SentenceSplitterException(
                            "Non-breaking prefix file for language '{}' was not found in package".format(
                                language
                            ))
                    SUBLIME_PACKAGE = True
                    _data = sublime.load_resource(non_breaking_prefix_file[0])
                    _data = _data.split('\n')
                    ...
                    print('! non_breaking_prefix_file', e)
        except Exception as e:
            print('non_breaking_prefix_file Exception', e)
            # Used for quick translation test outside SublineText before updating the plugin 
            SUBLIME_PACKAGE = False
        # VDI ^^^
